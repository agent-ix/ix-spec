...
| FR-012  | The CLI SHALL author the reserved OKF `index` and `log` archetypes through the same generic `write --types` catalog path as any other type (no reserved-name allowlist), resolving their skeleton/schema from a module that declares them (e.g. `spec-artifacts-iso`).                                                             | Test         |
| FR-013  | The CLI SHALL delegate all OKF bundle validation and index-completeness checking to quire (`quire validate --okf <bundle>` for foreign bundles; `quire validate --scope` for the owned repo) and SHALL NOT implement a separate ix-spec completeness lint; the authoring/validation boundary is ix-spec authors, quire validates.  | Review       |
