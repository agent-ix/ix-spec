...
| FR-006      | ✅ Covered | `flows.test.ts` exercises `specFlowNames`/`startSpecFlow`/`resolveSkillPath`/`runIxFlow` (real fake `ix-flow` binary: exit 0 / non-zero / signal / spawn-error, packaged + legacy layouts); `cli.test.ts` covers spec-flow dispatch through `main`.                                                         |
